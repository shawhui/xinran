        ...
        case UIGestureRecognizerStateEnded:
            switch (_positionMode) {
                ...
                case SpreadPositionModeTouchBorder:
                {
                    ...
                    CGFloat magneticDistance = 0;//superViewSize.height * MAGNETIC_SCOPE_RATIO_VERTICAL ;
                    CGPoint destinationLocation;
                    if (location.y < magneticDistance + 64) {//上面区域
                        if (location.x > superViewSize.width/2) {//右边
                            destinationLocation = CGPointMake(superViewSize.width - (self.bounds.size.width/2 + _touchBorderMargin), self.bounds.size.width/2 + _touchBorderMargin + 64);
                        } else if(location.x < magneticDistance) { //左边
                            destinationLocation = CGPointMake(self.bounds.size.width/2 + _touchBorderMargin, self.bounds.size.width/2 + _touchBorderMargin + 64);
                        } else {
                            destinationLocation = CGPointMake(location.x, self.bounds.size.width/2 + _touchBorderMargin + 64);
                        }
                    } else if (location.y > superViewSize.height - magneticDistance - 50) {//下面
                        if (location.x > superViewSize.width/2) {//右边
                            destinationLocation = CGPointMake(superViewSize.width - (self.bounds.size.width/2 + _touchBorderMargin), superViewSize.height - self.bounds.size.height/2 - _touchBorderMargin - 50);
                        } else if(location.x < magneticDistance) { //左边
                            destinationLocation = CGPointMake(self.bounds.size.width/2 + _touchBorderMargin, superViewSize.height - self.bounds.size.height/2 - _touchBorderMargin - 50);
                        } else {
                            destinationLocation = CGPointMake(location.x, superViewSize.height - self.bounds.size.height/2 - _touchBorderMargin - 50);
                        }
                    } else if (location.x > superViewSize.width/2) {//右边
                        if (location.y < magneticDistance + 64) {//上面
                            destinationLocation = CGPointMake(superViewSize.width - (self.bounds.size.width/2 + _touchBorderMargin),  self.bounds.size.width/2 + _touchBorderMargin + 64);
                        } else if (location.y > superViewSize.height - magneticDistance - 50) {//下面
                            destinationLocation = CGPointMake(superViewSize.width - (self.bounds.size.width/2 + _touchBorderMargin), superViewSize.height - self.bounds.size.height/2 - _touchBorderMargin - 50);
                        } else {
